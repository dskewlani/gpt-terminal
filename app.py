        ...
        with seg_tabs[1]:
            if not st.session_state[auto_key]:
                ...
                with mid:
                    ...
                    st.markdown(f'<div class="info-b">Minimum per line: ₹{eng.MIN_AUTO_TRADE_VALUE:,.0f} | CMP refresh: 5 seconds | Angel One first, fallback to exchange APIs.</div>', unsafe_allow_html=True)
                ...
                for pos in st.session_state[portfolio_key]:
                    ...
                    if use_trail and abs(pnl_pct) >= 1.2:
                        if pos.get("trailing_sl") is None:
                            pos["trailing_sl"] = ep
                        elif pos["type"] == "BUY":
                            pos["trailing_sl"] = max(pos["trailing_sl"], round(lp * 0.992, 2))
                        else:
                            pos["trailing_sl"] = min(pos["trailing_sl"], round(lp * 1.008, 2))
                    eff_sl = pos.get("trailing_sl") or pos.get("sl", 0)
                ...
                for i, pos in enumerate(st.session_state[portfolio_key]):
                    trail = f" | Trail SL ₹{pos['trailing_sl']:.2f}" if pos.get("trailing_sl") else ""
                    st.markdown(f"""
                    <div class="tc {'win' if pos.get('pnl',0) >= 0 else 'loss'}">
                    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                    <span class="tc-head">{pos['type']} {pos['symbol'].replace('.NS','').replace('.MCX','')} {pos.get('leverage',1)}x</span>
                    <span class="tc-meta">Entry ₹{pos['entry']:.2f} | CMP ₹{pos.get('cmp',pos['entry']):.2f} | Qty {pos['qty']} | Notional ₹{pos.get('invested',0):,.0f}{trail}</span>
                    {pnl_fmt(pos.get('pnl',0))}</div></div>""", unsafe_allow_html=True)
